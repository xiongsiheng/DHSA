...
const int MIN_SPARSE_K_BLOCK_SIZE = 32;
...
mha_varlen_fwd_block(at::Tensor &q,  // total_q x num_heads x head_size, total_q := \sum_{i=0}^{b} s_i
    ...
    if(has_blockmask){
        ...
        TORCH_CHECK(n_block_dim == 32 || n_block_dim == 64 || n_block_dim % SPARSE_SIZE == 0,
                    "n_block_dim must be 32, 64, or a multiple of 128");
    }
    if(has_streaming_info){
        TORCH_CHECK(streaming_info.dtype() == torch::kInt32, "streaming_info must have dtype int32");
        TORCH_CHECK(m_block_dim % SPARSE_SIZE == 0, "m_block_dim must be a multiple of 128");
        TORCH_CHECK(n_block_dim % SPARSE_SIZE == 0, "streaming n_block_dim must be a multiple of 128");
    }
    ...
    const int seqlen_k_round_base = has_blockmask ? (n_block_dim > MIN_SPARSE_K_BLOCK_SIZE ? n_block_dim : MIN_SPARSE_K_BLOCK_SIZE) : SPARSE_SIZE;
    const int seqlen_k_rounded = round_multiple(max_seqlen_k, seqlen_k_round_base);
...
#ifndef BLOCK_SPARSE_ATTN_FORWARD_ONLY
               ...
               std::optional<at::Tensor> &rng_state)
{
    ...
    if(has_blockmask){
        ...
        TORCH_CHECK(m_block_dim % SPARSE_SIZE == 0, "m_block_dim must be a multiple of 128");
        TORCH_CHECK(n_block_dim == 64 || n_block_dim % SPARSE_SIZE == 0,
                    "backward n_block_dim must be 64 or a multiple of 128");
    }
    if(has_streaming_info){
        TORCH_CHECK(streaming_info.dtype() == torch::kInt32, "streaming_info must have dtype int32");
        TORCH_CHECK(m_block_dim % SPARSE_SIZE == 0, "m_block_dim must be a multiple of 128");
    ...
    const int head_size_rounded = round_multiple(head_size, head_size <= 128 ? 32 : 64);
    const int seqlen_q_rounded = round_multiple(max_seqlen_q, SPARSE_SIZE);
    const int seqlen_k_rounded = round_multiple(max_seqlen_k, has_blockmask ? n_block_dim : SPARSE_SIZE);
...
#else
std::vector<at::Tensor>
mha_varlen_bwd_block(const at::Tensor &dout,
               const at::Tensor &q,
               const at::Tensor &k,
               const at::Tensor &v,
               const at::Tensor &out,
               ...
               std::optional<at::Tensor> &rng_state)
{
    TORCH_CHECK(false, "BlockSparseAttention backward is not available in this forward-only build");
}
#endif
} // namespace FLASH_NAMESPACE
