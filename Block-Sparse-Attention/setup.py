...
FORWARD_ONLY = os.getenv("BLOCK_SPARSE_ATTN_FORWARD_ONLY", "FALSE") == "TRUE"
LLAMA8B_BF16_CAUSAL_ONLY = os.getenv("BLOCK_SPARSE_ATTN_LLAMA8B_BF16_CAUSAL_ONLY", "FALSE") == "TRUE"
...
def add_cuda_gencodes(cc_flag, archs, bare_metal_version):
    ...
    # RTX 30xx / Ampere GA10x, e.g. RTX 3090.
    ...
    if bare_metal_version >= Version("12.8"):
        ...
        if "110" in archs:
            if bare_metal_version >= Version("13.0"):
                ...
                emitted_archs.append("110")
            else:
                # Provide Thor support for CUDA 12.9 via sm_101
                if bare_metal_version >= Version("12.8"):
                    cc_flag += ["-gencode", "arch=compute_101,code=sm_101"]
                    emitted_archs.append("101")
                # else: no Thor support in older toolkits

    # PTX for newest emitted arch. Do not request PTX for architectures that
    # this nvcc cannot parse, e.g. compute_120 with CUDA 12.4.
    if emitted_archs:
...
def cuda_include_dirs(cuda_home):
    candidates = []
    ...
    candidates.extend(cuda_candidates)

    site_roots = [Path(sys.prefix)]
    site_roots.extend(Path(path) for path in site.getsitepackages())
    for root in site_roots:
        candidates.extend(root.glob("**/site-packages/nvidia/*/include"))
...
if not SKIP_CUDA_BUILD:
    ...
    if FORWARD_ONLY:
        compiler_c17_flag.append("-DBLOCK_SPARSE_ATTN_FORWARD_ONLY")
        nvcc_flags.append("-DBLOCK_SPARSE_ATTN_FORWARD_ONLY")
    if LLAMA8B_BF16_CAUSAL_ONLY:
        compiler_c17_flag.append("-DBLOCK_SPARSE_ATTN_LLAMA8B_BF16_CAUSAL_ONLY")
        nvcc_flags.append("-DBLOCK_SPARSE_ATTN_LLAMA8B_BF16_CAUSAL_ONLY")
    ...
    ext_modules.append(
        CUDAExtension(
            ...
            sources=fwd_sources if FORWARD_ONLY else fwd_sources + bwd_sources,
            extra_compile_args={
